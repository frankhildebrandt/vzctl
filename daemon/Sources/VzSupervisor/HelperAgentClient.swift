    ...
    static func run(
        ...
        guard connected == 0 else {
            throw RouteApplyError.unavailable(
                "helper \(vmID) is unavailable at \(path)"
            )
        }
        ...
        guard writeAll(encoded, to: fd), let line = readLine(from: fd) else {
            throw RouteApplyError.unavailable("helper \(vmID) did not respond")
        }
        let response = try JSONRPCFraming.decode(JSONRPCResponse.self, from: line)
        if let error = response.error {
            switch error.code {
            case -32602:
                throw RouteApplyError.invalid(error.message)
            case -32018:
                throw RouteApplyError.unavailable(error.message)
            default:
                throw RouteApplyError.guest(error.message)
            }
        }
        guard let result = response.result else {
            throw RouteApplyError.guest("helper returned an empty response")
        }
        return result
    }
