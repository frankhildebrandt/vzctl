...
- Base-/Image-Pull zielt auf ARM64 Cloud/Server-Images (Aliases `ubuntu-latest`, `alpine-latest`, `arch-latest`, `fedora-latest`, `rocky-latest`, `alma-latest`), nicht Installer-ISOs; Workflow `pull → bake → seal`; Bake/Seal nutzen lokales `virt-customize` oder die gepinnte Builder-Appliance (`scripts/build-builder-appliance.sh`, Cache unter `images/builder/`).
